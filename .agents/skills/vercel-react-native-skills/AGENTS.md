...
```tsx
interface Item { id: string, text?: string, imageUrl?: string, isHeader?: boolean }
...
interface MessageItem { id: string, type: 'message', text: string }
interface ImageItem { id: string, type: 'image', url: string }
...
```tsx
interface Props { fallbackEnabled: boolean }
