...
function processDriverArray(drivers: Driver[], rules: CarClassRule[]): ProcessedEntry[] {
    ...
    drivers.forEach(driver => {
        const driverName = driver.CarInfo ? driver.CarInfo.DriverName : 'Unknown';
        ...
        if (driver.Cars) {
            Object.keys(driver.Cars).forEach(carModel => {
                ...
                if (carData.BestSplits) {
                    const splitKeys = Object.keys(carData.BestSplits).sort((a, b) => parseInt(a) - parseInt(b));
                ...
                if (carData.BestLapSplits) {
                    const splitKeys = Object.keys(carData.BestLapSplits).sort((a, b) => parseInt(a) - parseInt(b));
                    splitKeys.forEach(key => {
                ...
                results.push({
                    ...
                    bestLap: bestLapMs,
                    splits: splits,
                    bestLapSplits: bestLapSplits,
                    theoreticalBestLap: theoreticalBestLap,
